...
lr_feature_selection_p <- function(X,ignoresamecolprefix = FALSE)
{
  ...
  singlecols<- subset(singlecols,col1_pvalue !='NaN' & col1_pvalue != 'NA')
...
chiindepndencetestforfactcols <- function(X){
  ...
  #removefirst val
  pvals<- subset(pvals,col1_pvalue !='NaN' & col1_pvalue != 'NA')
  pvals$col1_pvalue <- as.numeric(as.character(pvals$col1_pvalue))
  pvals <- pvals[-1,]
  list(colpvals= pvals)
}
