...
# 7. Before generating the graphs it samples the data so that max rows are mxrows like 20000 as plotting too much data will be time consuming
...
prelimprocessing <- function(X,resrows=2,inttonum=TRUE,clscol='Class',timetovieweachinput=5,generategraphs=TRUE,remmissingdatabeforeprocessing=FALSE,maxlvl=60,cutoff=0.001,mxrowforplotting=20000)
{
  ...
  #Use startified splitting. Use just mxrowforplotting rows for graphs as too much data will cause issues
  test_ratio <- mxrowforplotting/nrow(X)
